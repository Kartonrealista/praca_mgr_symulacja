...
fn main() {
    let path1 =
        "/home/kartonrealista/actual_code/praca_mgr_symulacja/ptau1000.csv";
    let path2 =
        "/home/kartonrealista/actual_code/praca_mgr_symulacja/stezenia.csv";
    let _path1win = r"C:\Users\admin\Desktop\MTHOMAS\x\ptau1000.csv";
    let _path2win = r"C:\Users\admin\Desktop\MTHOMAS\x\stezenia.csv";
    ...
    //równania
    let eq1 =
        move |c: &[f64; 15], _c_h2o2: &f64, c_ho2min: &f64, _c_scn: &f64| {
    ...
    let eq4 =
        move |c: &[f64; 15], _c_h2o2: &f64, _c_ho2min: &f64, _c_scn: &f64| {
            k_m3 * c[3] - k_m20 * c[4] * c[9]
        };
    let eq5 =
        move |c: &[f64; 15], c_h2o2: &f64, _c_ho2min: &f64, _c_scn: &f64| {
            k_6prim * c[3] - k_m21 * c[5] * c[8] + k_m3 * c[3]
                - 2.0 * k_m4 * c[5] * c[5]
                + 2.0 * k_oh2 * c_h2o2 * c[12]
                - 2.0 * k_phen1 * c[5] * c[5] * c[14]
        };
    let eq6 =
        move |c: &[f64; 15], c_h2o2: &f64, _c_ho2min: &f64, c_scn: &f64| {
    ...
    let eq12 =
        move |c: &[f64; 15], c_h2o2: &f64, c_ho2min: &f64, _c_scn: &f64| {
...
fn rk4(
    ...
    let kxconculator =
        |kxs: &[f64; 15], multiplier, kxconcs: &mut [f64; 15]| {
            (0usize..15)
                .zip(kxs)
...
fn potencjal_mieszany(
    ...
    let i02pt =
        10.0_f64.powf(-5.0) * F * c_ho2min.powf(0.5) * c_ho2rod.powf(0.5);
    let i02au =
        10.0_f64.powf(-8.0) * F * c_ho2min.powf(0.5) * c_ho2rod.powf(0.5);

    let e_pt = R * T / F
        * ((i01 * (F * e1 / 2.0 / R / T).exp()
            + i02pt * (F * e2 / 2.0 / R / T).exp())
            / (i01 * (-F * e1 / 2.0 / R / T).exp()
                + i02pt * (-F * e2 / 2.0 / R / T).exp()))
        .ln();
    let e_au = R * T / F
        * ((i01 * (F * e1 / 2.0 / R / T).exp()
            + i02au * (F * e2 / 2.0 / R / T).exp())
            / (i01 * (-F * e1 / 2.0 / R / T).exp()
                + i02au * (-F * e2 / 2.0 / R / T).exp()))
        .ln();
    (e_au, e_pt)
